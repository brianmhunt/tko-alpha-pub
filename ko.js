...
const PotentialWeakMap = window.WeakMap || Map;
const bindingHandlerClassMap = new PotentialWeakMap();
...
function createBindingHandlerClass (handler, key) {
...
function getBindingHandlerClass (handler, key, onError) {
  let HandlerClass = bindingHandlerClassMap.get(handler);
  if (!HandlerClass) {
    HandlerClass = createBindingHandlerClass(handler, key);
    bindingHandlerClassMap.set(handler, HandlerClass);
  }
  return HandlerClass
}
...
class ElseBindingHandler extends ConditionalBindingHandler {
  ...
  get elseChainIsAlreadySatisfied () {
    ...
    const conditionObject = get(node, 'conditional') || {};
